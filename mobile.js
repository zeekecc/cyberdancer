...
function initParticleCanvas(){ particleCanvas.width=window.innerWidth; particleCanvas.height=window.innerHeight; }
...
function triggerComboFlash(){
  ...
  flashTimeout=setTimeout(()=>{ comboFlash.classList.remove('active'); flashTimeout=null; },120);
}
...
function updateColorPreview(scheme){
  ...
  squares.forEach((div,index)=>{ if(index<colors.length) div.style.background=colors[index]; });
}
...
function buildStaticLayer(){
  ...
  for(let i=1;i<LANE_COUNT;i++){ let xPos=i*LANE_WIDTH; staticCtx.beginPath(); staticCtx.moveTo(xPos,0); staticCtx.lineTo(xPos,canvas.height); staticCtx.stroke(); }
...
function drawArcadeArrow(ctx,x,y,width,height,lane,fillColor,strokeColor,isReceptor=false,isHitFlash=false){
  ...
  if(!drawArcadeArrow.cache[cacheKey]){
    ...
    const dotSpacing=5;
    const dotSize=2;
    octx.globalAlpha=isUnpressedReceptor?0.2:(isHitFlash?1.0:0.6);
    for(let dx=-w;dx<=w;dx+=dotSpacing){ for(let dy=-h;dy<=h;dy+=dotSpacing){ octx.fillRect(dx-dotSize/2,dy-dotSize/2,dotSize,dotSize); } }
    octx.restore();

    if(!isUnpressedReceptor){ octx.shadowBlur=isHitFlash?30:(isReceptor?20:15); octx.shadowColor=glowColor; }
  ...
  for(let i=noteStartIndex;i<notes.length;i++){
    ...
    if(note.holding){
      ...
      if(currentSongTime>=holdEndTime){
        ...
        spawnParticles(screenX,screenY,ARROW_COLORS[note.lane],12);
    ...
    if(note.hit){
      if(note.hitAnim>0){ drawArcadeArrow(ctx,(note.lane*LANE_WIDTH)+6,RECEPTOR_Y-32,LANE_WIDTH-12,64,note.lane,'#ffffff',ARROW_COLORS[note.lane],false,true); note.hitAnim--; }
      continue;
    }
